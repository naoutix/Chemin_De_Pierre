...
points = np.loadtxt("../data/gaussian_3dpoints.txt")
G = gen_graph(points, delta_max=10)

shape = int(np.sqrt(points.shape[0]))

cvt_nodes = np.reshape(np.arange(points.shape[0]), (shape,shape))

depart = np.array([round(shape/2), round(shape/2)]) #np.random.randint(shape,size=2)
fin = np.array([100, 100]) #np.random.randint(shape,size=2)
...
np.save("../data/chemin_gaussian.npy",np.array([x_list,y_list]))
...
plt.savefig("../data/chemin_gaussian.png", dpi=150)
plt.show()
